/*
 * SyncNoteBeta.js  —  CLICK-TO-JUMP BETA (toolbar function: SyncNoteBeta)
 * Revives card-wide click navigation (v0.8.x) with selection-aware text.
 * Fold into SyncNote.js on approval; safe to run beside the stable build.
 * -----------------------------------------------------------------------------
...
function SyncNoteBeta() {
  ...
  function buildDialog(model, layer) {
    ...
    var clickJumpArmed = 0;   // BETA: filters armed this refresh (diagnostic)
    var lastJumpMs = 0;       // BETA: one jump per click — a click bubbles
    ...
    function makeGroupWidget(group) {
      ...
      // Links stay as-is, but this BETA also revives card-wide click-to-
      // jump (v0.8.x; the flakiness was the GC bug, fixed by pinning) —
      ...
      // ---- click-to-jump (BETA): the group card's background jumps too.
    ...
    function makeNoteCard(drawingName, frameNo, note) {
      ...
      // ---- click-to-jump (BETA): the whole note card is a jump target.
    ...
    // ---- click-to-jump (BETA — revival of v0.8.x) -----------------------
